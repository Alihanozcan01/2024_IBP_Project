...
class Director extends BaseController
{
    ...
    function addShopPost(Request $request) {
        ...
        if(!$Shop){
            return redirect(route('addShop'))->with('error', 'Mağaza kayıt işlemi gerçekleştirilemedi, tekrar deneyiniz');
        }
        return redirect(route('addShop'))->with('success', 'Mağaza başarıyla kayıt edildi');
    }

    function updateShopPost(Request $request) {
        $validatedData = $request->validate([
            'name' => 'required|string|exists:Shops,name',
        ...
        if (is_null($Shop)) {
            ...
            return redirect(route('updateShop'))->with('error', 'Girdiğiniz Mağaza sistemde kayıtlı değil.');
        }
        ...
        return redirect(route('updateShop'))->with('success', 'Mağazain bilgileri başarıyla güncellendi.');
    }
